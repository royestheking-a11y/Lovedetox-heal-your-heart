...
export function NoContactMonitor() {
  ...
  const loadMessages = () => {
    const stored = JSON.parse(localStorage.getItem('noContactMessages') || '[]');
    setMessages(stored);
  };
                ...
                return (
                  <div key={email} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-3 bg-white dark:bg-gray-800 rounded-lg">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold self-start sm:self-center ${
                      state === 'critical' ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'
                    }`}>
            ...
            <div key={email} className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-2xl border border-gray-200 dark:border-gray-700">
              <div className="flex items-start gap-4">
                ...
                <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center flex-shrink-0 ${
                  getUserMentalState(email) === 'critical' || getUserMentalState(email) === 'warning' ? 'bg-red-100 dark:bg-red-900/30' :
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 sm:gap-4 mb-3">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold self-start ${
                      getUserMentalState(email) === 'critical' ? 'bg-red-100 text-red-700' :
