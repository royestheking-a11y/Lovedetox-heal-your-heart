...
export function GuestChatModal({ onClose, onSignup }: GuestChatModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl max-w-2xl w-full h-[600px] flex flex-col relative">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-[#4B0082]">AI Emotional Support</h4>
              <p className="text-sm text-gray-500">Guest Preview ({10 - messageCount} messages left)</p>
            </div>
          </div>
          <button
            onClick={onClose}
            ...
            >
              <div
                className={`max-w-[80%] px-4 py-3 rounded-2xl ${message.role === 'user'
                  ? 'bg-gradient-to-r from-[#4B0082] to-[#FF8DAA] text-white'
                  : 'bg-gray-100 text-gray-800'
                  }`}
                style={message.role === 'user' ? { color: 'white' } : {}}
              >
                <p className={`text-sm ${message.role === 'user' ? '!text-white' : ''}`} style={message.role === 'user' ? { color: 'white' } : {}}>{message.content}</p>
              </div>
            </div>
          ))}
          {isTyping && (
          ...
          {messageCount < 10 ? (
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-[#FF8DAA] focus:outline-none transition-colors bg-white text-gray-900"
              />
              <button
                onClick={handleSend}
                disabled={!input.trim() || isTyping}
                className="px-6 py-3 gradient-primary text-white rounded-xl hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed"
              >
            ...
            <div className="text-center">
              <p className="text-gray-600 mb-4">Guest preview limit reached</p>
              <button
                onClick={onSignup}
